...
ds_do = 1; % Ratio of swirl arm to outlet radius, (Recommend 2-5)
...
d_i_outer = 4 / 1000; % Inlet diameter (unimportant parameter?)
...
rho_fu = py.CoolProp.CoolProp.PropsSI('D','T', Tl_fu,'P', Pman_fu, fuel); % Fu density in manifold, [kg/m^3]
...
while abs(error_outer) > tolerance_outer && i < 100
    ...
    % Find flow areas
    A_eff = mdot_fu / (sqrt(2 * rho_fu * dP_fu) * Cd_orifices);
    A_o = pi / 4 * d_o ^ 2; 
    ...
    % Find viscous cone angle
    alpha_actual = atand(2 * Cd * K_guess / sqrt(abs((1 + S) ^ 2 - 4 * Cd ^ 2 * K_guess ^ 2))) * 2; % (Eq. 5-75, Beyvel and Orzechowski)
...
plot((r_i_outer + fu_start + r_i_outer*cos(theta)) * unit_convert, (r_o_outer + r_i_outer + r_i_outer*sin(theta)) * unit_convert, 'Linewidth', 1.5) % Fuel orifice
...
str = {
    ...
    'Post Gap [mm]: ' + string(round(gap * unit_convert,2)),...
